...
'''
sys.argv[5] is to be used when universal adapters that don't
include barcodes are being tested, in this case the input will
be an integer representing which header part to use to account
for the difference in expected read length (e.g.; if using R1
read, specify 5, and if R2, specify 4) currently, a buffer size
of 6 is hardcoded under buffer
'''

if sys.argv[5]:
    modifier2 = int(sys.argv[5])
    buffer = 6
...
with open(fastq1) as trimmed, open(fastq2) as original:
    for line1, line2 in zip(trimmed, original):
        ...
        if i == 1:
            ...
            if modifier2:
                exp_len += buffer + len(line1.split(':')[modifier2])
