...
see_the_calls <- function(working_d, platform){
  setwd(working_d)
  ...
  group = rep(platform, length(miscalls$call))
  miscalls = cbind(miscalls, group)
  miscalls = subset(miscalls, miscalls$empirical_qscore != "NaN")
  write.csv(miscalls, file = 'miscalls_only.csv', row.names = FALSE)
  return(miscalls)
}

plot_machine <- function(w_d, miscalls){
  base_plot <- ggplot() +
    ...
    ggtitle(paste("Base-calling Error Rates (", platform, ")", sep=""))
