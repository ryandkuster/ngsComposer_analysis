...
def krill_main():
    ...
    pe_1 = os.path.join(proj, 'pe.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in1))
    se_1 = os.path.join(proj, 'se.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in1))
    f_1 = os.path.join(proj, 'failed.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in1))
    if in2:
        pe_2 = os.path.join(proj, 'pe.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in2))
        se_2 = os.path.join(proj, 'se.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in2))
        f_2 = os.path.join(proj, 'failed.' + '_p' + str(q_percent) + 'q' + str(q_min) + '_' + os.path.basename(in2))
        krill_open(q_min, q_percent, p64, in1, in2, pe_1, pe_2, se_1, se_2, f_1, f_2)
    else:
        krill_single_open(q_min, q_percent, p64, in1, se_1, f_1)


def krill_open(q_min, q_percent, p64, in1, in2, pe_1, pe_2, se_1, se_2, f_1, f_2):
    '''
...
def krill(q_min, q_percent, p64, f1, f2, pe_o1, pe_o2, se_o1, se_o2, fail_o1, fail_o2):
    ...
    for line1, line2 in zip(f1, f2):
        ...
        if y == 4:
            ...
            if krill1 is False and krill2 is False:
                ...
                se_o1.write(entry1)
                fail_o2.write(entry2)
            elif krill1 is True and krill2 is False:
                fail_o1.write(entry1)
                se_o2.write(entry2)
...
def krill_single_open(q_min, q_percent, p64, in1, se_1, f_1):
    '''
    ...
    if compressed:
        se_1, _ = os.path.splitext(se_1)
        f_1, _ = os.path.splitext(f_1)
        with gzip.open(in1, 'rt') as f1, open(se_1, "w") as se_o1, open(f_1, "w") as fail_o1:
            krill_single(q_min, q_percent, p64, f1, se_o1, fail_o1)
    else:
        with open(in1) as f1, open(se_1, "w") as se_o1, open(f_1, "w") as fail_o1:
            krill_single(q_min, q_percent, p64, f1, se_o1, fail_o1)


def krill_single(q_min, q_percent, p64, f1, se_o1, fail_o1):
    scores = open(os.path.dirname(os.path.abspath(__file__)) +
                  '/helpers/scores.txt').read().split()
    if p64:
        val = dict(zip(scores[31:95], range(0, 43)))
